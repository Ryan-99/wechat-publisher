...
def _attach_title(soup, content) -> None:
    ...
    content._article_title = title_tag.get_text(strip=True) if title_tag else ""
...
def fetch_article(url: str, timeout: int = 20) -> "BeautifulSoup tag":
    ...
    The article title is attached as ``content._article_title`` (empty string
    if not found).
    """
...
def main():
    ...
    title = getattr(content, "_article_title", "")
    if title:
        print(f"Title: {title}")
