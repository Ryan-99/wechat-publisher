...
走浏览器即梦（jimeng.jianying.com），无需 API key。Phase 4/5 时打开即梦页面生成图片。
...
按 visual.md 的结构化模板写提示词，每条末尾附加 Phase 4 的视觉锚点。通过浏览器即梦批量生成，插入到 markdown 对应段落后。

1. 在即梦页面逐个输入提示词（16:9 比例）
2. 下载图片到 `{dir}/output/illust-{N}.png`
3. 在 markdown 中对应段落后插入 `![](output/illust-{N}.png)`

**降级**：即梦不可用时，将所有提示词保存到 `{dir}/output/illustration-prompts.txt`。
...
| 封面生成 | 浏览器即梦 → 保存提示词让用户手动生成 |
| 配图生成 | 浏览器即梦 → 保存提示词让用户手动生成 |
